...
void LightPool::init() {
    ...
    lightImpostorShader.setPropertyDescriptor(fragModule);
...
void LightPool::drawLightImpostors(RenderStage stage, GFX::CommandBuffer& bufferInOut) const {
    ...
    if (totalLightCount > 0) {
        ...
        GFX::BindDescriptorSetsCommand descriptorSetCmd;
        descriptorSetCmd._set._textureData.setTexture({ _lightIconsTexture->getHandle(), 0u, _lightIconsTexture->getTextureType() }, to_U8(ShaderProgram::TextureUsage::UNIT0));
        GFX::EnqueueCommand(bufferInOut, descriptorSetCmd);
