    ...
    : Renderer(context, cache, RendererType::RENDERER_TILED_FORWARD_SHADING)
{
    ...
    params._primitiveCount = maxNumLightsPerTile * numTiles;
    params._primitiveSizeInBytes = sizeof(U32);
    params._ringBufferLength = 1;
    params._unbound = true;
    params._updateFrequency = BufferUpdateFrequency::ONCE;

    _perTileLightIndexBuffer = _context.gfx().newSB(params);
...
void TiledForwardShadingRenderer::preRender(RenderTarget& target, LightPool& lightPool) {
    ...
    target.bind(to_const_U8(ShaderProgram::TextureUsage::DEPTH), RTAttachment::Type::Depth, 0);
...
U32 TiledForwardShadingRenderer::getMaxNumLightsPerTile() const {
    ...
    U16 height = std::min(_resolution.height, to_const_U16(1080));
