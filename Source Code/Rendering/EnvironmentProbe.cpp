...
vectorEASTL<bool> EnvironmentProbe::s_availableSlices;
...
void EnvironmentProbe::debugDraw(GFX::CommandBuffer& bufferInOut) {
    ...
    constants.set(_ID("dvd_WorldMatrixOverride"), GFX::PushConstantType::MAT4, mat4<F32>(bbPos.x, bbPos.y, bbPos.z));
    constants.set(_ID("dvd_LayerIndex"), GFX::PushConstantType::UINT, to_U32(_currentArrayIndex));
    ...
    if (!_boundingBoxPrimitive) {
        ...
        _boundingBoxPrimitive->name(Util::StringFormat("EnvironmentProbe_%d", getGUID()).c_str());
