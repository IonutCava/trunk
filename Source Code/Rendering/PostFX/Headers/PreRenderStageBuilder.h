...
public:
   ...
   PreRenderOperator* 	 addGenericOperator(Shader* const shader, Quad3D* const target, bool& state, FrameBufferObject* result);
   PreRenderOperator* 	 addSSAOOperator(Shader* const SSAOShader, Quad3D* const target, bool& state, FrameBufferObject* result);
   PreRenderOperator*    addBloomOperator(Shader* const bloomShader, Quad3D* const target, bool& state, FrameBufferObject* result);
   PreRenderOperator*    addDeferredShadingOperator(Shader* const deferredShader, Quad3D* const target, bool& state, FrameBufferObject* result);
