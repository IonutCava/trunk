...
~PreRenderStageBuilder();
/// Adding PreRenderOperators to the PreRender stage, needs an input shader to
/// be applied to the scene/target
/// A target fullscreen quad to which it should render it's output. This output
/// is saved in the "result" FB
///"state" is a reference to the global variable that enables or disables the
///effect (via options, PostFX, config, etc)
template <typename T>
inline PreRenderOperator* addPreRenderOperator(bool& state, Framebuffer* result,
                                               const vec2<U16>& resolution) {
    return addToStage(MemoryManager_NEW T(result, resolution, _screenSampler),
                      state);
}
