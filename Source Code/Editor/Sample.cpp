...
namespace Divide {
    ...
    void NodeWindow::OnGui()
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_ChildWindowBg, ImVec4(60 / 255.0f, 60 / 255.0f, 70 / 255.0f, 200 / 255.0f));
        ...
        // Open context menu
        if (!ImGui::IsAnyItemHovered() && ImGui::IsAnyWindowHovered() && ImGui::IsMouseClicked(1))
        {
    ...
    MyImwWindow3::MyImwWindow3(const char* pTitle)
    {
        SetTitle(pTitle);
    }

    void MyImwWindow3::OnGui()
    {
        if (!IsClosable())
        {
    ...
    MyImwWindow2::MyImwWindow2(const char* pTitle)
    {
        SetTitle(pTitle);
    }

    void MyImwWindow2::OnGui()
    {
        static float f = 0.0f;
        ImGui::SliderFloat("float", &f, 0.0f, 1.0f);
    }

    MyImwWindowFillSpace::MyImwWindowFillSpace()
    {
        SetTitle("Filling space");
        SetFillingSpace(true);
    }

    void MyImwWindowFillSpace::OnGui()
    {
        ImVec2 oSize = ImGui::GetContentRegionAvail();
        ImGui::Selectable("I'm a selectable\nwho fills the whole space", true, 0, oSize);
    }

    MyImwWindow::MyImwWindow(const char* pTitle )
        : ImwMenu(0, false)
    {
        SetTitle(pTitle);
        SetAlone(true);
    ...
    void MyImwWindow::OnContextMenu()
    {
