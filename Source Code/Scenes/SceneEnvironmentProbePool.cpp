...
EnvironmentProbe_wptr SceneEnvironmentProbePool::addInfiniteProbe(const vec3<F32>& position) {
    EnvironmentProbe_ptr probe = eastl::make_shared<EnvironmentProbe>(parentScene(), EnvironmentProbe::ProbeType::TYPE_INFINITE);
...
EnvironmentProbe_wptr SceneEnvironmentProbePool::addLocalProbe(const vec3<F32>& bbMin,
    ...
    EnvironmentProbe_ptr probe = eastl::make_shared<EnvironmentProbe>(parentScene(), EnvironmentProbe::ProbeType::TYPE_LOCAL);
