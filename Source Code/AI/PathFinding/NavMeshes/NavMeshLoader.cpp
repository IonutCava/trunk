...
namespace Navigation {
	...
	void NavigationMeshLoader::addVertex(NavModelData* modelData, const vec3<F32>& vertex){
		...
		*dst++ = vertex.z;
		...
		PRINT_FN("ADDING VERTEX: %5.2f, %5.2f, %5.2f",vertex.x,vertex.y,vertex.z);
	...
	void NavigationMeshLoader::addTriangle(NavModelData* modelData, const vec3<U32>& triangleIndices,const SamplePolyAreas& areaType){
	...
	NavModelData NavigationMeshLoader::parseNode(SceneGraphNode* sgn, const std::string& navMeshName){
	...
	static const vec3<F32> cubePoints[8] = {   vec3<F32>(-1.0f, -1.0f, -1.0f),
