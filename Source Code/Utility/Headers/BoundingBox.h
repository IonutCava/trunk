...
public:
	...
	inline vec3<F32>  getMin()	     const	{return _min;}
	inline vec3<F32>  getMax()		 const	{return _max;}
	...
	inline void setMin(const vec3<F32>& min)			    {_min = min; _dirty = true;}
	inline void setMax(const vec3<F32>& max)			    {_max = max; _dirty = true;}
	inline void set(const vec3<F32>& min, const vec3<F32>& max)  {_min = min; _max = max; _dirty = true;}
