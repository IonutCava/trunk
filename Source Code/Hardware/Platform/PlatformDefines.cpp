...
void* operator new( size_t size ) {
	...
	Divide::log_new( ptr, size, " allocation outside of macro ", 0 );
	return ptr;
}

void  operator delete( void *ptr ) {
	Divide::log_delete( ptr );
	free( ptr );
}

void* operator new[]( size_t size ) {
	void* ptr = malloc( size );
	Divide::log_new( ptr, size, " array allocation outside of macro ", 0 );
	return ptr;
}
...
void* operator new( size_t size, char* zFile, int nLine ){
	void* ptr = malloc( size );
	Divide::log_new( ptr, size, zFile, nLine );
	return ptr;
}
...
void* operator new[]( size_t size, const char* pName, Divide::I32 flags, Divide::U32 debugFlags, const char* file, Divide::I32 line ) {
	void* ptr = malloc( size );
	//Divide::log_new( ptr, size, file, line );
	return ptr;
}
