...
GLbyte GL_API::initHardware(const vec2<GLushort>& resolution, GLint argc, char **argv){
    ...
    _useMSAA = (par.getParam<GLubyte>("rendering.FSAAmethod",FS_MSAA) == FS_MSAA ||
                par.getParam<GLubyte>("rendering.FSAAmethod",FS_MSAA) == FS_MSwFXAA) && (msaaSamples > 1);
    ...
    if(GLEW_ARB_debug_output) {
        ...
        glDebugMessageCallbackARB(&Divide::GL::DebugCallbackARB, (GLvoid*)(0));
...
#   ifdef _DEBUG
    if(GLEW_ARB_debug_output) {
        glEnable(GL_DEBUG_OUTPUT_SYNCHRONOUS_ARB);
        glDebugMessageCallbackARB(&Divide::GL::DebugCallbackARB, (GLvoid*)(1));
    }
#   endif
#endif
