...
{ 
	...
	fh = fopen( filename, "r" );
	if ( fh == NULL ){
		clPrint( stderr, "Error on loadProgramSource");
...
cl_program _sclCreateProgram( char* program_source, cl_context context )
{
	...
	if ( err!=CL_SUCCESS ) {
		clPrint( stderr,  "Error on createProgram" );
...
cl_kernel _sclCreateKernel( sclSoft software ) {
	...
	if ( err != CL_SUCCESS ) {
		clPrint( stderr,  "Error on createKernel %s ", software.kernelName );
		sclPrintErrorFlags( err );
	}
#else
...
void sclPrintHardwareStatus( sclHard hardware ) {
	...
	err = clGetPlatformInfo( hardware.platform,
			...
			platform,
			NULL );
	if ( err == CL_SUCCESS ) { clPrint( stdout, "\nPlatform object alive" ); }
