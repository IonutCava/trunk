...
// default constructor
CDDSImage::CDDSImage() :
        m_format(Format::COUNT), m_components(0), m_type(TextureNone), m_valid(false) {
}

CDDSImage::~CDDSImage() {
}

void CDDSImage::create_textureFlat(Format format, unsigned int components, const CTexture &baseImage) {
...
void CDDSImage::create_texture3D(Format format, unsigned int components, const CTexture &baseImage) {
    assert(format != Format::COUNT);
    assert(components != 0);
    assert(baseImage.get_depth() > 1);
...
void CDDSImage::create_textureCubemap(Format format, unsigned int components, const CTexture &positiveX, const CTexture &negativeX,
...
void CDDSImage::load(istream& is, bool flipImage) {
    ...
    if (ddsh.ddspf.dwFlags & DDSF_FOURCC) {
        ...
        case FOURCC_DXT1:
            m_format = Format::DXT1;
            m_components = 3;
            break;
        case FOURCC_DXT3:
            m_format = Format::DXT3;
            m_components = 4;
            break;
        case FOURCC_DXT5:
            m_format = Format::DXT5;
            m_components = 4;
            break;
        default:
    ...
    for (unsigned int n = 0; n < (unsigned int) (m_type == TextureCubemap ? 6 : 1); n++) {
        ...
        for (unsigned int i = 0; i < numMipmaps && (w || h); i++) {
            ...
            uint8_t *pixels2 = new uint8_t[size];
            is.read((char*)pixels2, size);

            mipmap.create(w, h, d, size, pixels2);
...
void CDDSImage::save(const std::string& filename, bool flipImage) {
    ...
    if (is_compressed()) {
        ...
        if (m_format == Format::DXT3)
            ddsh.ddspf.dwFourCC = FOURCC_DXT3;
        if (m_format == Format::DXT5)
...
inline unsigned int CDDSImage::size_dxtc(unsigned int width, unsigned int height) {
    return ((width + 3) / 4) * ((height + 3) / 4) * (m_format == Format::DXT1 ? 8 : 16);
}
