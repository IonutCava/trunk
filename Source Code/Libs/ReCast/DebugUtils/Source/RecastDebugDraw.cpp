                        ...
                        const unsigned char* flags, const float texScale)
{
    ...
    for (int i = 0; i < ntris*3; i += 3)
    {
        ...
        if (flags && !flags[i/3])
            colour = duLerpCol(duRGBA(a,a,a,255), unwalkable, 64);
        else
            colour = duRGBA(a,a,a,255);
        ...
        dd->vertex(va, colour, uva);
        dd->vertex(vb, colour, uvb);
        dd->vertex(vc, colour, uvc);
    }
    dd->end();
    dd->texture(false);
}
...
void duDebugDrawHeightfieldSolid(duDebugDraw* dd, const rcHeightfield& hf)
{
    ...
    unsigned int fcol[6];
    duCalcBoxColours(fcol, duRGBA(255,255,255,255), duRGBA(255,255,255,255));
    ...
    for (int y = 0; y < h; ++y)
    {
        for (int x = 0; x < w; ++x)
        {
            const float fx = orig[0] + x*cs;
            const float fz = orig[2] + y*cs;
...
void duDebugDrawHeightfieldWalkable(duDebugDraw* dd, const rcHeightfield& hf)
{
    ...
    unsigned int fcol[6];
    duCalcBoxColours(fcol, duRGBA(255,255,255,255), duRGBA(217,217,217,255));
        ...
        for (int x = 0; x < chf.width; ++x)
        {
            ...
            for (unsigned i = c.index, ni = c.index+c.count; i < ni; ++i)
            {
                ...
                if (chf.areas[i] == RC_WALKABLE_AREA)
                    ...
                    colour = duIntToCol(chf.areas[i], 255);
                
                const float fy = chf.bmin[1] + (s.y+1)*ch;
                dd->vertex(fx, fy, fz, colour);
                dd->vertex(fx, fy, fz+cs, colour);
                dd->vertex(fx+cs, fy, fz+cs, colour);
                dd->vertex(fx+cs, fy, fz, colour);
            }
        }
    }
    dd->end();
    ...
    for (int y = 0; y < chf.height; ++y)
    {
        for (int x = 0; x < chf.width; ++x)
        {
            ...
            for (unsigned i = c.index, ni = c.index+c.count; i < ni; ++i)
            {
                ...
                const unsigned int colour = duRGBA(cd,cd,cd,255);
                dd->vertex(fx, fy, fz, colour);
                dd->vertex(fx, fy, fz+cs, colour);
                dd->vertex(fx+cs, fy, fz+cs, colour);
                dd->vertex(fx+cs, fy, fz, colour);
...
static void drawLayerPortals(duDebugDraw* dd, const rcHeightfieldLayer* layer)
{
    ...
    const unsigned int pcol = duRGBA(255,255,255,255);
...
void duDebugDrawHeightfieldLayer(duDebugDraw* dd, const struct rcHeightfieldLayer& layer, const int idx)
{
    ...
    const unsigned int colour = duIntToCol(idx+1, 255);
    ...
    duDebugDrawBoxWire(dd, bmin[0],bmin[1],bmin[2], bmax[0],bmax[1],bmax[2], duTransCol(colour,128), 2.0f);
...
void duDebugDrawRegionConnections(duDebugDraw* dd, const rcContourSet& cset, const float alpha)
{
    ...
    const unsigned int colour = duRGBA(0,0,0,196);
    ...
    for (int i = 0; i < cset.nconts; ++i)
    {
        ...
        for (int j = 0; j < cont->nverts; ++j)
        {
            ...
            if (cont2)
            {
                ...
                duAppendArc(dd, pos[0],pos[1],pos[2], pos2[0],pos2[1],pos2[2], 0.25f, 0.6f, 0.6f, colour);
            }
        }
    }
    ...
    {
        const rcContour* cont = &cset.conts[i];
        const unsigned int col = duDarkenCol(duIntToCol(cont->reg,a));
        getContourCenter(cont, orig, cs, ch, pos);
        dd->vertex(pos, col);
    }
    dd->end();
...
void duDebugDrawRawContours(duDebugDraw* dd, const rcContourSet& cset, const float alpha)
{
    ...
    for (int i = 0; i < cset.nconts; ++i)
    {
        ...
        const unsigned int colour = duIntToCol(c.reg, a);

        for (int j = 0; j < c.nrverts; ++j)
        {
            const int* v = &c.rverts[j*4];
            const float fx = orig[0] + v[0]*cs;
            const float fy = orig[1] + (v[1]+1+(i&1))*ch;
            const float fz = orig[2] + v[2]*cs;
            dd->vertex(fx,fy,fz,colour);
            if (j > 0)
                dd->vertex(fx,fy,fz,colour);
        }
    ...
    {
        const rcContour& c = cset.conts[i];
        const unsigned int colour = duDarkenCol(duIntToCol(c.reg, a));
        
        for (int j = 0; j < c.nrverts; ++j)
        {
            const int* v = &c.rverts[j*4];
            float off = 0;
            unsigned int colv = colour;
            ...
            const float fx = orig[0] + v[0]*cs;
            const float fy = orig[1] + (v[1]+1+(i&1))*ch + off;
            const float fz = orig[2] + v[2]*cs;
...
void duDebugDrawContours(duDebugDraw* dd, const rcContourSet& cset, const float alpha)
{
    ...
    for (int i = 0; i < cset.nconts; ++i)
    {
        ...
        if (!c.nverts)
            continue;
        const unsigned int colour = duIntToCol(c.reg, a);
        const unsigned int bcolour = duLerpCol(colour,duRGBA(255,255,255,a),128);
        for (int j = 0, k = c.nverts-1; j < c.nverts; k=j++)
        {
            const int* va = &c.verts[k*4];
            const int* vb = &c.verts[j*4];
            const unsigned int col = (va[3] & RC_AREA_BORDER) ? bcolour : colour;
            float fx,fy,fz;
            fx = orig[0] + va[0]*cs;
...
void duDebugDrawPolyMesh(duDebugDraw* dd, const struct rcPolyMesh& mesh)
{
    ...
    for (int i = 0; i < mesh.npolys; ++i)
    {
        ...
        if (mesh.areas[i] == RC_WALKABLE_AREA)
            ...
            colour = duIntToCol(mesh.areas[i], 255);
...
void duDebugDrawPolyMeshDetail(duDebugDraw* dd, const struct rcPolyMeshDetail& dmesh)
{
    ...
    for (int i = 0; i < dmesh.nmeshes; ++i)
    {
        ...
        const unsigned int colour = duIntToCol(i, 192);

        for (int j = 0; j < ntris; ++j)
        {
            dd->vertex(&verts[tris[j*4+0]*3], colour);
            dd->vertex(&verts[tris[j*4+1]*3], colour);
            dd->vertex(&verts[tris[j*4+2]*3], colour);
        }
    }
    dd->end();
    ...
    for (int i = 0; i < dmesh.nmeshes; ++i)
    {
        ...
        for (int j = 0; j < ntris; ++j)
        {
            ...
            {
                const unsigned char ef = (t[3] >> (kp*2)) & 0x3;
                if (ef == 0)
                {
                    // Internal edge
                    if (t[kp] < t[k])
