...
void tcp_session_tpl::start_read()
{
    ...
    boost::asio::async_read(socket_, boost::asio::buffer(&header,sizeof(header)),
        _strand.wrap(boost::bind(&tcp_session_tpl::handle_read_body, shared_from_this(), _1,
...
void tcp_session_tpl::handle_read_body(const boost::system::error_code& ec,size_t bytes_transfered)
{
    ...
    if (!ec)
    {
        ...
        boost::asio::async_read(socket_, input_buffer_.prepare(header),
            _strand.wrap(boost::bind(&tcp_session_tpl::handle_read_packet, shared_from_this(), _1,
                        boost::asio::placeholders::bytes_transferred)));
    }
    else
    {
...
void tcp_session_tpl::handle_read_packet(const boost::system::error_code& ec,size_t bytes_transfered)
{
    if (stopped())
      return;
...
void tcp_session_tpl::start_write()
{
    ...
    if(p.getOpcode() == SMSG_SEND_FILE)
    {
        boost::asio::async_write(socket_,buffers,
        _strand.wrap(boost::bind(&tcp_session_tpl::handle_write_file, shared_from_this(), _1)));
    }
    else
    {
        boost::asio::async_write(socket_,buffers,
            _strand.wrap(boost::bind(&tcp_session_tpl::handle_write, shared_from_this(), _1)));
    }
}
...
void tcp_session_tpl::await_output()
 {
    ...
    if(output_queue_.empty())
    {
        ...
        {
            non_empty_output_queue_.expires_at(boost::posix_time::pos_infin);
            non_empty_output_queue_.async_wait(boost::bind(&tcp_session_tpl::await_output, shared_from_this()));
        }
    }
    else
    {
...
void tcp_session_tpl::check_deadline(deadline_timer* deadline)
{
    ...
    else
    {
      ...
      deadline->async_wait(
          boost::bind(&tcp_session_tpl::check_deadline,
...
udp_broadcaster::udp_broadcaster(boost::asio::io_service& io_service,
                                 const udp::endpoint& broadcast_endpoint) : socket_(io_service)
{
