...
void Client::handle_read_body(const boost::system::error_code& ec,size_t bytes_transfered){
    if (stopped_)  return;
...
void Client::handle_read_packet(const boost::system::error_code& ec,size_t bytes_transfered){
    if (stopped_) return;
    ...
    if (!ec) {
		...
		if(packet.getOpcode() == SMSG_SEND_FILE){
			boost::asio::async_read_until(socket_,
				request_buf, "\n\n",
				boost::bind(&Client::handle_read_file,
...
void Client::handle_read_file(const boost::system::error_code& ec,size_t bytes_transfered){
    std::cout << __FUNCTION__ << "(" << bytes_transfered << ")"
        ...
        async_read(socket_, boost::asio::buffer(buf.c_array(), buf.size()),
            boost::bind(&Client::handle_read_file_content,
            this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
    }
