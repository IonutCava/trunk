...
int balloc (bstring b, int olen) {
    int len;
    if (b == nullptr || b->data == nullptr || b->slen < 0 || b->mlen <= 0 ||
    ...
    if (olen >= b->mlen) {
        ...
        if (7 * b->mlen < 8 * b->slen) {
            ...
            if (x == nullptr) {
                ...
                if (nullptr == (x = (unsigned char *) bstr__realloc (b->data, (size_t) (len = olen)))) {
                    return BSTR_ERR;
                }
            }
        } else {
            ...
            if (nullptr == (x = (unsigned char *) bstr__alloc ((size_t) len))) {
 ...
 */
int ballocmin (bstring b, int len) {
    if (b == nullptr || b->data == nullptr || (b->slen+1) < 0 || b->mlen <= 0 ||
        b->mlen < b->slen || len <= 0) {
        return BSTR_ERR;
    }
    ...
    if (len != b->mlen) {
        unsigned char* s = (unsigned char*)bstr__realloc(b->data, (size_t) len);
        if (nullptr == s) return BSTR_ERR;
...
bstring bfromcstr (const char * str) {
    ...
    size_t j = (strlen)(str);
    int i = snapUpSize((int)(j + (2 - (j != 0))));
    if (i <= (int) j) return nullptr;

    bstring b = (bstring)bstr__alloc(sizeof (struct tagbstring));
    if (nullptr == b) return nullptr;
    b->slen = (int) j;
    if (nullptr == (b->data = (unsigned char *) bstr__alloc (b->mlen = i))) {
        bstr__free (b);
        return nullptr;
    }
 ...
 */
bstring blk2bstr (const void * blk, int len) {
    if (blk == nullptr || len < 0) return nullptr;
    bstring b = (bstring)bstr__alloc(sizeof (struct tagbstring));
    if (b == nullptr) return nullptr;
    b->slen = len;
...
char * bstr2cstr (const_bstring b, char z) {
    if (b == nullptr || b->slen < 0 || b->data == nullptr) return nullptr;
    int l = b->slen;
    char* r = (char*)bstr__alloc((size_t) (l + 1));
...
int bconcat (bstring b0, const_bstring b1) {
    ...
    if (b0 == nullptr || b1 == nullptr || b0->data == nullptr || b1->data == nullptr) return BSTR_ERR;
    ...
    if (b0->mlen <= d + len + 1) {
        ...
        if (0 <= pd && pd < b0->mlen) {
            if (nullptr == (aux = bstrcpy (b1))) return BSTR_ERR;
        }
...
int bcatcstr (bstring b, const char * s) {
    ...
    if (b == nullptr || b->data == nullptr || b->slen < 0 || b->mlen < b->slen
     || b->mlen <= 0 || s == nullptr) return BSTR_ERR;

    /* Optimistically concatenate directly */
    int l = b->mlen - b->slen;
    char* d = (char*)&b->data[b->slen];
    for (i=0; i < l; i++) {
        if ((*d++ = *s++) == '\0') {
            b->slen += i;
            return BSTR_OK;
...
int nl;
    ...
    if (b == nullptr || b->data == nullptr || b->slen < 0 || b->mlen < b->slen
     || b->mlen <= 0 || s == nullptr || len < 0) return BSTR_ERR;
...
bstring bstrcpy (const_bstring b) {
    ...
    bstring b0 = (bstring)bstr__alloc(sizeof (struct tagbstring));
    if (b0 == nullptr) {
 ...
 */
int bassign (bstring a, const_bstring b) {
    if (b == nullptr || b->data == nullptr || b->slen < 0)
        return BSTR_ERR;
    if (b->slen != 0) {
        if (balloc (a, b->slen) != BSTR_OK) return BSTR_ERR;
        bstr__memmove (a->data, b->data, b->slen);
    } else {
        if (a == nullptr || a->data == nullptr || a->mlen < a->slen ||
            a->slen < 0 || a->mlen == 0)
            return BSTR_ERR;
    }
...
int bassignblk (bstring a, const void * s, int len) {
    if (a == nullptr || a->data == nullptr || a->mlen < a->slen ||
        a->slen < 0 || a->mlen == 0 || nullptr == s || len + 1 < 1)
        return BSTR_ERR;
 ...
 */
int btrunc (bstring b, int n) {
    if (n < 0 || b == nullptr || b->data == nullptr || b->mlen < b->slen ||
...
int btoupper (bstring b) {
int i, len;
    if (b == nullptr || b->data == nullptr || b->mlen < b->slen ||
        b->slen < 0 || b->mlen <= 0) return BSTR_ERR;
...
int v, n;
    ...
    if (bdata (b0) == nullptr || b0->slen < 0 ||
        bdata (b1) == nullptr || b1->slen < 0) return SHRT_MIN;
...
int v;
    ...
    if (bdata (b0) == nullptr || b0->slen < 0 ||
        bdata (b1) == nullptr || b1->slen < 0 || n < 0) return SHRT_MIN;
    int m = n;
 ...
 */
int bisstemeqcaselessblk (const_bstring b0, const void * blk, int len) {
    if (bdata (b0) == nullptr || b0->slen < 0 || nullptr == blk || len < 0)
        return BSTR_ERR;
 ...
 */
int biseq (const_bstring b0, const_bstring b1) {
    if (b0 == nullptr || b1 == nullptr || b0->data == nullptr || b1->data == nullptr ||
...
int biseqcstr (const_bstring b, const char * s) {
int i;
    if (b == nullptr || s == nullptr || b->data == nullptr || b->slen < 0) return BSTR_ERR;
    for (i=0; i < b->slen; i++) {
...
int bstrcmp (const_bstring b0, const_bstring b1) {
    ...
    int n = b0->slen; if (n > b1->slen) n = b1->slen;
...
int bdelete (bstring b, int pos, int len) {
    ...
    if (len < 0 || b == nullptr || b->data == nullptr || b->slen < 0 ||
 ...
 */
int bdestroy (bstring b) {
    if (b == nullptr || b->slen < 0 || b->mlen <= 0 || b->mlen < b->slen ||
        b->data == nullptr)
        return BSTR_ERR;
...
if (b1 == nullptr || b1->data == nullptr || b1->slen < 0 ||
        b2 == nullptr || b2->data == nullptr || b2->slen < 0) return BSTR_ERR;
 ...
 */
int bstrchrp (const_bstring b, int c, int pos) {
    if (b == nullptr || b->data == nullptr || b->slen <= pos || pos < 0) return BSTR_ERR;
    unsigned char* p = (unsigned char*)bstr__memchr((b->data + pos), (unsigned char) c, (b->slen - pos));
...
static int buildCharField (struct charField * cf, const_bstring b) {
    if (b == nullptr || b->data == nullptr || b->slen <= 0) return BSTR_ERR;
...
static void invertCharField (struct charField * cf) {
    for (int i = 0; i < CFCLEN; i++) cf->content[i] = ~cf->content[i];
}
...
struct charField chrs;
    if (pos < 0 || b0 == nullptr || b0->data == nullptr ||
...
static int binchrrCF (const unsigned char * data, int pos, const struct charField * cf) {
    for (int i = pos; i >= 0; i--) {
        const unsigned int c = (unsigned int) data[i];
        if (testInCharField (cf, c)) return i;
    }
    return BSTR_ERR;
}
...
int binchrr (const_bstring b0, int pos, const_bstring b1) {
struct charField chrs;
    if (pos < 0 || b0 == nullptr || b0->data == nullptr || b1 == nullptr ||
        b0->slen < pos) return BSTR_ERR;
    if (pos == b0->slen) pos--;
...
bstring aux = (bstring) b1;
    ...
    if (pos < 0 || b0 == nullptr || b0->slen < 0 || nullptr == b0->data ||
        b0->mlen < b0->slen || b0->mlen <= 0) return BSTR_ERR;
    if (b1 != nullptr && (b1->slen < 0 || b1->data == nullptr)) return BSTR_ERR;
...
bstring aux = (bstring) b2;
    ...
    if (pos < 0 || len < 0 || (pl = pos + len) < 0 || b1 == nullptr ||
        b2 == nullptr || b1->data == nullptr || b2->data == nullptr ||
...
bstring auxr = (bstring) repl;
    ...
    int mlen = 32;
    int* d = (int*)static_d; /* Avoid malloc for trivial cases */
    acc = slen = 0;
    ...
    while ((pos = instr (b, pos, auxf)) >= 0) {
        if (slen + 1 >= mlen) {
            int * t;
            mlen += mlen;
            int sl = sizeof(int*) * mlen;
            if (static_d == d) d = nullptr;
            if (sl < mlen || nullptr == (t = (int *) bstr__realloc (d, sl))) {
                ret = BSTR_ERR;
                goto done;
            }
            if (nullptr == d) bstr__memcpy (t, static_d, sizeof (static_d));
            d = t;
        }
        d[slen] = pos;
        slen++;
...
int i;
    ...
    if (pos < 0 || b == nullptr || b->slen < 0 || b->mlen < b->slen ||
...
int bpattern (bstring b, int len) {
    ...
    if (len > 0) {
        if (d == 1) return bsetstr (b, len, nullptr, b->data[0]);
        for (int i = d; i < len; i++) b->data[i] = b->data[i - d];
    }
    b->data[len] = (unsigned char) '\0';
    b->slen = len;
    return BSTR_OK;
 ...
 */
int breada (bstring b, bNread readPtr, void * parm) {
    if (b == nullptr || b->mlen <= 0 || b->slen < 0 || b->mlen < b->slen ||
        readPtr == nullptr) return BSTR_ERR;

    int i = b->slen;
    for (int n = i + 16; ; n += ((n < BS_BUFF_SZ) ? n : BS_BUFF_SZ)) {
        if (BSTR_OK != balloc (b, n + 1)) return BSTR_ERR;
        int l = (int)readPtr((void*)(b->data + i), 1, n - i, parm);
        i += l;
        b->slen = i;
        if (i < n) break;
    }
...
/*  bstring bread_gl (bNread readPtr, void * parm)
 *
...
int c;
    ...
    if (b == nullptr || b->mlen <= 0 || b->slen < 0 || b->mlen < b->slen ||
        getcPtr == nullptr) return BSTR_ERR;
    int d = 0;
    int e = b->mlen - 2;
 ...
 */
struct bStream * bsopen (bNread readPtr, void * parm) {
    if (readPtr == nullptr) return nullptr;
    struct bStream* s = (struct bStream*)bstr__alloc(sizeof (struct bStream));
 ...
 */
int bsbufflength (struct bStream * s, int sz) {
    if (s == nullptr || sz < 0) return BSTR_ERR;
    int oldSz = s->maxBuffSz;
    if (sz > 0) s->maxBuffSz = sz;
    return oldSz;
}

int bseof (const struct bStream * s) {
    if (s == nullptr || s->readFnPtr == nullptr) return BSTR_ERR;
...
struct charField cf;
    ...
    if (s == nullptr || s->buff == nullptr || r == nullptr || term == nullptr ||
        term->data == nullptr || r->mlen <= 0 || r->slen < 0 ||
    ...
    int l = s->buff->slen;
    if (BSTR_OK != balloc (s->buff, s->maxBuffSz + 1)) return BSTR_ERR;
    unsigned char* b = (unsigned char*)s->buff->data;
    x.data = b;
...
int bsreadlns (bstring r, struct bStream * s, const_bstring term) {
    if (s == nullptr || s->buff == nullptr || r == nullptr || term == nullptr
     || term->data == nullptr || r->mlen <= 0) return BSTR_ERR;
 ...
 */
int bsunread (struct bStream * s, const_bstring b) {
    if (s == nullptr || s->buff == nullptr) return BSTR_ERR;
...
bstring bjoin (const struct bstrList * bl, const_bstring sep) {
    ...
    if (bl == nullptr || bl->qty < 0) return nullptr;
    if (sep != nullptr && (sep->slen < 0 || sep->data == nullptr)) return nullptr;

    for (i = 0, c = 1; i < bl->qty; i++) {
        v = bl->entry[i]->slen;
        if (v < 0) return nullptr;    /* Invalid input */
        c += v;
        if (c < 0) return nullptr;    /* Wrap around ?? */
    }

    if (sep != nullptr) c += (bl->qty - 1) * sep->slen;

    bstring b = (bstring)bstr__alloc(sizeof (struct tagbstring));
    if (nullptr == b) return nullptr; /* Out of memory */
...
int i, p, ret;
    ...
    if (cb == nullptr || s == nullptr || s->readFnPtr == nullptr
     || splitStr == nullptr || splitStr->slen < 0) return BSTR_ERR;

    if (nullptr == (buff = bfromcstr (""))) return BSTR_ERR;
 ...
 */
int bstrListDestroy (struct bstrList * sl) {
    if (sl == nullptr || sl->qty < 0) return BSTR_ERR;
...
int bstrListAllocMin (struct bstrList * sl, int msz) {
    ...
    if (sl->mlen == msz) return BSTR_OK;
    size_t nsz = ((size_t)msz) * sizeof(bstring);
...
int i, ret;
    ...
    if (cb == nullptr || str == nullptr || pos < 0 || pos > str->slen)
        return BSTR_ERR;
...
struct genBstrList * g = (struct genBstrList *) parm;
    if (g->bl->qty >= g->bl->mlen) {
        ...
        bstring* tbl = (bstring*)bstr__realloc(g->bl->entry, sizeof (bstring) * mlen);
        if (tbl == nullptr) return BSTR_ERR;
...
struct genBstrList g;
    ...
    if (     str == nullptr ||      str->slen < 0 ||      str->data == nullptr ||
        splitStr == nullptr || splitStr->slen < 0 || splitStr->data == nullptr)
        return nullptr;
...
int n, r;
    ...
    if (b == nullptr || fmt == nullptr || b->data == nullptr || b->mlen <= 0
    ...
    if (nullptr == (buff = bfromcstralloc (n + 2, ""))) {
        n = 1;
        if (nullptr == (buff = bfromcstralloc (n + 2, ""))) return BSTR_ERR;
    }
...
int n, r;
    ...
    if (nullptr == (buff = bfromcstralloc (n + 2, ""))) {
        n = 1;
        if (nullptr == (buff = bfromcstralloc (n + 2, ""))) return nullptr;
    }
...
int n, r, l;
    ...
    if (b == nullptr || fmt == nullptr || count <= 0 || b->data == nullptr
