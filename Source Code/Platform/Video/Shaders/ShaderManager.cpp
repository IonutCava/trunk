...
/// Initialize the shader manager so that it may process program loading
/// requests
bool ShaderManager::init() {
    ...
    // Create an immediate mode rendering shader that simulates the fixed
    // function pipeline
...
bool ShaderManager::recompileShaderProgram(const stringImpl& name) {
    ...
    for (ShaderProgramMap::value_type& it : _shaderPrograms) {
        ...
        if (shaderName.find(name) != stringImpl::npos ||
            shaderName.compare(name) == 0) {
            // We process every partial match. So add it to the recompilation
            // queue
...
/// Bind the NULL shader which should have the same effect as using no shaders
/// at all
