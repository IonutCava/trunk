...
*/
Task* CreateTask(PlatformContext& context, const DELEGATE<void, Task&>& threadedFunction, bool allowedInIdle)
{
    return CreateTask(context.taskPool(TaskPoolType::HIGH_PRIORITY), threadedFunction, allowedInIdle);
}

Task* CreateTask(PlatformContext& context, Task* parentTask, const DELEGATE<void, Task&>& threadedFunction, bool allowedInIdle)
{
