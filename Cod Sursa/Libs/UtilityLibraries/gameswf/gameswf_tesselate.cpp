...
namespace tesselate
{
	...
	// More Renderer state.
	static SWF::array<fill_segment>	s_current_segments;	// @@ should not dynamically resize this thing!
	static SWF::array<point>	s_current_path;			// @@ should not dynamically resize this thing!
	...
	// trapezoids.  Modifies the values in s_current_segments.
	{
		...
		// Peel off first.
		SWF::array<fill_segment>	slab;	// @@ make this use static storage
		for (int i = i0; i < i1; i++)
		{
