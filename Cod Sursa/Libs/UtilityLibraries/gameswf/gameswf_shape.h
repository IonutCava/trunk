...
namespace gameswf
{
	...
	private:
		SWF::array<Sint16>	m_triangle_strip;
	};
...
//		int	m_last_frame_rendered;	// @@ Hm, we shouldn't spontaneously drop cached data I don't think...
		...
		SWF::array<mesh>	m_meshes;	// One mesh per style.
		SWF::array<line_strip>	m_line_strips;
	};
	...
	// information on fill and line styles.
	{
		...
		const SWF::array<fill_style>&	get_fill_styles() const { return m_fill_styles; }
		const SWF::array<line_style>&	get_line_styles() const { return m_line_styles; }
		const SWF::array<path>&	get_paths() const { return m_paths; }
	...
	protected:
		...
		SWF::array<fill_style>	m_fill_styles;
		SWF::array<line_style>	m_line_styles;
		SWF::array<path>	m_paths;
	...
	private:
		...
		mutable SWF::array<mesh_set*>	m_cached_meshes;
	};
