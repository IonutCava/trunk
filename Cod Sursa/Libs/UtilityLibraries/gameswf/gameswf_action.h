...
namespace gameswf
{
	...
	struct action_buffer
	{
		...
		void	execute(
			...
			int exec_bytes,
			as_value* retval,
			const SWF::array<with_stack_entry>& initial_with_stack,
			bool is_function2);
	...
	private:
		// Don't put these as values in SWF::array<>!  They contain
		...
		// data:
		SWF::array<unsigned char>	m_buffer;
		SWF::array<const char*>	m_dictionary;
...
// (though I'm not sure).
	...
	struct as_as_function : public ref_counted
	{
		...
		SWF::array<with_stack_entry>	m_with_stack;	// initial with-stack on function entry.
		int	m_start_pc;
		int	m_length;
		struct arg_spec
		{
		...
		// were defined.
		as_as_function(action_buffer* ab, as_environment* env, int start, const SWF::array<with_stack_entry>& with_stack)
			:
			m_action_buffer(ab),
			m_env(env),
	...
	struct as_environment
	{
		SWF::array<as_value>	m_stack;
		as_value	m_global_register[4];
		SWF::array<as_value>	m_local_register;	// function2 uses this
		movie*	m_target;
		...
		SWF::array<frame_slot>	m_local_frames;
		...
		as_value	get_variable(const tu_string& varname, const SWF::array<with_stack_entry>& with_stack) const;
		// no path stuff:
		as_value	get_variable_raw(const tu_string& varname, const SWF::array<with_stack_entry>& with_stack) const;

		void	set_variable(const tu_string& path, const as_value& val, const SWF::array<with_stack_entry>& with_stack);
		// no path stuff:
		void	set_variable_raw(const tu_string& path, const as_value& val, const SWF::array<with_stack_entry>& with_stack);
