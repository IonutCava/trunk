...
namespace gameswf
{
	...
	void	write_coord_array(tu_file* out, const SWF::array<Sint16>& pt_array)
	...
	void	read_coord_array(tu_file* in, SWF::array<Sint16>* pt_array)
...
//		memcpy(&m_coords[0], coords, coord_count * sizeof(coords[0]));
	...
	struct tri_stripper
	{
		...
		SWF::array< SWF::array<point> >	m_strips;
		...
		// Add two triangles to our strip.
		{
			...
			if (l0.bitwise_equal(r0) == false)
			{
				...
				for ( ; i < n; i++)
				{
					SWF::array<point>&	str = m_strips[i];
		...
		// under the given style.
		{
			if (m_strips.size())
			{
				...
				for (int i = 1, n = m_strips.size(); i < n; i++)
				{
					...
					const SWF::array<point>&	str = m_strips[i];
					assert(str.size() >= 3);	// should have at least one tri already.
...
//		m_last_frame_rendered(0),
		m_error_tolerance(error_tolerance)
	{
		struct collect_traps : public tesselate::trapezoid_accepter
		{
			...
			// strips-in-progress.
			SWF::hash<int, tri_stripper*>	m_strips;
			...
			{
				for (SWF::hash<int, tri_stripper*>::const_iterator it = m_strips.begin();
				     it != m_strips.end();
				     ++it)
				{
					// Push strip into m.
...
//	void	mesh_set::set_last_frame_rendered(int frame_counter) { m_last_frame_rendered = frame_counter; }
	...
	void	mesh_set::display(
		const matrix& mat,
		const cxform& cx,
		const SWF::array<fill_style>& fills,
		const SWF::array<line_style>& line_styles) const
	...
	void	mesh_set::display(
		const matrix& mat,
		const cxform& cx,
		const SWF::array<morph_fill_style>& fills,
		const SWF::array<morph_line_style>& line_styles,
		float ratio) const
	// Throw our meshes at the renderer.
	{
		assert(m_error_tolerance > 0);
	...
	static void	read_fill_styles(SWF::array<fill_style>* styles, stream* in, int tag_type, movie_definition_sub* m)
	...
	static void	read_line_styles(SWF::array<line_style>* styles, stream* in, int tag_type)
...
// << Thomas
	...
	static void	debug_display_shape_paths(
		...
		const SWF::array<path>& paths,
		const SWF::array<fill_style>& fill_styles,
		const SWF::array<line_style>& line_styles)
	{
