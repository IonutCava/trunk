...
//		}
	...
	// Handy for external binding.
	{
		...
		SWF::array<with_stack_entry>	dummy_with_stack;
	...
	struct key_as_object : public as_object
	{
		...
		SWF::array<weak_ptr<as_object_interface> >	m_listeners;
...
#define KEY_CONST(k) key_obj->set_member(#k, key::k)
	...
	// The is_function2 flag determines whether to use global or local registers.
	{
		...
		SWF::array<with_stack_entry>	with_stack(initial_with_stack);
...
//            log_error("get url2: target=%s, url=%s\n", target, url);
	...
	as_value	as_environment::get_variable(const tu_string& varname, const SWF::array<with_stack_entry>& with_stack) const
	...
	as_value	as_environment::get_variable_raw(
		...
		const SWF::array<with_stack_entry>& with_stack) const
	...
	void	as_environment::set_variable(
		const tu_string& varname,
		const as_value& val,
		const SWF::array<with_stack_entry>& with_stack)
