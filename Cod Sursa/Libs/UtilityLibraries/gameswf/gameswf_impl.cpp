...
namespace gameswf
{
	...
	static SWF::hash<int, loader_function>	s_tag_loaders;
	...
	static void	execute_actions(as_environment* env, const SWF::array<action_buffer*>& action_list)
	...
	{
		SWF::hash<int, smart_ptr<character_def> >	m_characters;
		SWF::hash<int, smart_ptr<font> >	m_fonts;
		SWF::hash<int, smart_ptr<bitmap_character_def> >	m_bitmap_characters;
		SWF::hash<int, smart_ptr<sound_sample> >	m_sound_samples;
		SWF::array<SWF::array<execute_tag*> >	m_playlist;	// A list of movie control events for each frame.
		SWF::array<SWF::array<execute_tag*> >	m_init_action_list;	// Init actions for each frame.
		stringi_hash<int>	m_named_frames;	// 0-based frame #'s
		stringi_hash<smart_ptr<resource> >	m_exports;

		// Items we import.
		SWF::array<import_info>	m_imports;

		// Movies we import from; hold a ref on these, to keep them alive
		SWF::array<smart_ptr<movie_definition> >	m_import_source_movies;

		// Bitmaps used in this movie; collected in one place to make
		// it possible for the host to manage them as textures.
		SWF::array<smart_ptr<bitmap_info> >	m_bitmap_list;
		...
		virtual const SWF::array<execute_tag*>&	get_playlist(int frame_number) { return m_playlist[frame_number]; }

		/* movie_def_impl */
		virtual const SWF::array<execute_tag*>*	get_init_actions(int frame_number)
		{
		...
		/* movie_def_impl */
		void	get_owned_fonts(SWF::array<font*>* fonts)
		// Fill up *fonts with fonts that we own.
		{
			assert(fonts);
			fonts->resize(0);

			SWF::array<int>	font_ids;

			for (SWF::hash<int, smart_ptr<font> >::iterator it = m_fonts.begin();
			     it != m_fonts.end();
			     ++it)
			{
		...
		// Dump our cached data into the given stream.
		{
			...
			{for (SWF::hash<int, smart_ptr<character_def> >::iterator it = m_characters.begin();
			     it != m_characters.end();
			     ++it)
			{
	...
	struct movie_root : public movie_interface
	{
		...
		SWF::array<Timer *>	m_interval_timers;
	...
	static SWF::hash< movie_definition_sub*, smart_ptr<movie_interface> >	s_movie_library_inst;
	static SWF::array<movie_interface*> s_extern_sprites;
...
//	 			m->add_bitmap_character(character_id, ch);
	...
	struct swf_event
	{
		...
		void	read(stream* in, Uint32 flags)
		{
			...
			SWF::array<with_stack_entry>	empty_with_stack;
	...
	struct place_object_2 : public execute_tag
	{
		...
		SWF::array<swf_event*>	m_event_handlers;
	...
	struct sprite_definition : public movie_definition_sub
	{
		...
		SWF::array<SWF::array<execute_tag*> >	m_playlist;	// movie control events for each frame.
		...
		const SWF::array<execute_tag*>&	get_playlist(int frame_number)
	...
	struct sprite_instance : public character
	{
		...
		SWF::array<action_buffer*>	m_action_list;
		...
		SWF::array<bool>	m_init_actions_executed;	// a bit-array class would be ideal for this
...
//			return m_def->get_character_def(character_id);
		...
		// frame is 0-based
		{
			...
			{
				const SWF::array<execute_tag*>*	init_actions = m_def->get_init_actions(frame);
			...
			const SWF::array<execute_tag*>&	playlist = m_def->get_playlist(frame);
		...
		execute_tag*	find_previous_replace_or_add_tag(int frame, int depth, int id)
		{
			...
			{
				const SWF::array<execute_tag*>&	playlist = m_def->get_playlist(f);
...
//			IF_VERBOSE_DEBUG(log_msg("sprite::goto_frame(%d)\n", target_frame_number));//xxxxx
		...
		character*	add_display_object(
			Uint16 character_id,
			const char* name,
			const SWF::array<swf_event*>& event_handlers,
			Uint16 depth,
...
//				val->set(0.0);
		...
		// frame_spec could be an integer or a string.
		{
			...
			// Execute the actions.
			const SWF::array<execute_tag*>&	playlist = m_def->get_playlist(frame_number);
			for (int i = 0; i < playlist.size(); i++)
			{
				execute_tag*	e = playlist[i];
				if (e->is_action_tag())
		...
		// at a new depth.
		{
			...
			if (ch)
			{
				SWF::array<swf_event*>	dummy_event_handlers;
		...
		virtual void	attach_display_callback(const char* path_to_object, void (*callback)(void*), void* user_ptr)
		{
			...
			SWF::array<with_stack_entry>	dummy;
