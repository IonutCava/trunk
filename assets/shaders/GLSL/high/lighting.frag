...
void main (void){
	...
	vec4 color;
	//if we are drawing to depth map, color all objects in red for better visibility during debugging
	if(mode == MODE_DEPTH)
	...
	//Else, use appropriate lighting / bump mapping / relief mapping / normal mapping
		...
		color = vec4(vNormalMV.xyz,1);
	

	//Apply opacity even during depth rendering to handle transparent objects
	if(mode != MODE_SSAO){
		if(hasOpacity ){
			vec4 alpha = texture2D(opacityMap, gl_TexCoord[0].st*tile_factor);
...
float ShadowMapping(out vec3 vPixPosInDepthMap){
	...
	for(int i = 0; i < 3; i++){
		if(!ok){
			...
			vPixPosInDepthMap = vec3(gl_TexCoord[1].xy/tOrtho[i], gl_TexCoord[1].z) / (gl_TexCoord[1].w);
	...
	if(ok){
		switch(id){
			case 0:
				fShadow = filterFinalShadow(texDepthMapFromLight0,posInDM, 2048);
				break;
			case 1:
				fShadow = filterFinalShadow(texDepthMapFromLight1,posInDM, 1024);
				break;
			case 2:
				fShadow = filterFinalShadow(texDepthMapFromLight2,posInDM, 512);
				break;
		};
	}
