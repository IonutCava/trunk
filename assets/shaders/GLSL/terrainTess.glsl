...
void main(void)
{
    ...
    const vec4 posAndTileScaleVert = tData._positionAndTileScale;
    vtx_tileScale = int(posAndTileScaleVert.w);
    vtx_adjacency = tData._tScale;

    dvd_Vertex = vec4(vec3(u, 0.0f, v) * vtx_tileScale + posAndTileScaleVert.xyz, 1.0f);
