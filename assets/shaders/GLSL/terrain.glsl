...
{	
	...
	float final_detail = detail_scale;
	float final_diffuse = diffuse_scale;
	if (LOD == 0){
		final_detail = detail_scale * 2;
		final_diffuse = diffuse_scale * 2;
	}else if (LOD == 2){
		final_detail = detail_scale / 2;
		final_diffuse = diffuse_scale / 2;
	}
	vec2 uv_detail = uv * final_detail;
	vec2 uv_diffuse = uv * final_diffuse;
